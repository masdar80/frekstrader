...
class PositionSizer:
    ...
    def calculate(
        ...
        # 2. Derive Lot Size
        # Value of 1 pip for standard lot (typically $10 for a standard 100,000 contract if USD quote)
        contract_size = symbol_info.get("contract_size", 100000)
        point_value = symbol_info.get("point", 0.00001)
        
        # Example logic (assuming USD quote pairs like EURUSD, GBPUSD):
        # Tick value = Pip value for 1 point * contract size
        tick_value = point_value * contract_size
        ...
        # Volume calculation
        required_volume = risk_amount / (sl_points * tick_value)
